 ...
 * @author A40385
 */
@WebServlet(urlPatterns = {"/getAllApexConsoles"})
public class GetAllApexConsoles extends HttpServlet {
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if(userID == null){
                ...
                ArrayList<Platform> platformList = platformDBManager.getHadoopPlatforms();
                ...
                for(Platform currentPlatform : platformList){
                    ...
                    jsonResult.put(Key.HADOOP, currentPlatform.getURL()); 
                    platformJSONList.put(platformJSON);
                } 
                
                platformList = platformDBManager.getYarnPlatforms();
                for(Platform currentPlatform : platformList){
                    JSONObject platformJSON = new JSONObject();
                    platformJSON.put(Key.TYPE, currentPlatform.getType());
                    platformJSON.put(Key.URL, currentPlatform.getURL());
                    platformJSON.put(Key.PROJECT_NAME, currentPlatform.getProjectName());
                    platformJSON.put(Key.USER, currentPlatform.getUserID());
                    jsonResult.put(Key.YARN, currentPlatform.getURL()); 
                    platformJSONList.put(platformJSON);
                } 
